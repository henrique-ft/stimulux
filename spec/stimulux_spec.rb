...
RSpec.describe Stimulux do
  ...
  describe '#stimulus_controller' do
    it 'returns controller data attributes' do
      ...
      expect(stimulus_controller('hello', ['foo_bar', { 'someThing' => 'xpto' }])).to eq(expected)
    end

    it 'handles only names' do
      expected = { 'data-controller' => 'hello world' }
      expect(stimulus_controller('hello', 'world')).to eq(expected)
    end

    it 'handles values for multiple stimulus_controller' do
      expected = {
        'data-controller' => 'c1 c2',
        'data-c1-my-value-value' => '1',
        'data-c2-other-value-value' => '2'
      }
      expect(stimulus_controller(['c1', { myValue: '1' }], ['c2', { other_value: '2' }])).to eq(expected)
    end
  end
  ...
  describe '#stimulus_target' do
    it 'returns target data attributes' do
      ...
      expect(stimulus_target('hello#name', 'foo-bar#baz')).to eq(expected)
    end

    it 'returns target data attributes' do
      expected = { 'data-hello-target' => 'myName', 'data-foo-bar-target' => 'baz' }
      expect(stimulus_target('hello#myName', 'foo-bar#baz')).to eq(expected)
    end
  end
  ...
  describe '#stimulus_action' do
    it 'returns action data attributes' do
      ...
      expect(stimulus_action('hello#greet', 'foo-bar#doSomething')).to eq(expected)
    end
  end
  ...
  describe '#stimulus_class' do
    it 'returns class data attributes' do
      ...
      expect(stimulus_class(['foo_bar', { 'noResults' => 'bg-gray-500' }],
